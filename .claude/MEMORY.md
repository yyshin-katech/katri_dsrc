...
- 동작: DSRC RSU 로부터 UDP(:60000) 수신 → 16바이트 벤더 헤더 뒤 SAE J2735 PER 디코드 → SPaT 평탄화(`parseSpat`) → `SIG_SPAT[7]` 를 1024바이트 UDP 로 `127.0.0.1:50000` 송신 → 하류 ROS 노드(mcar `katri_obu_interface`) 소비.
...
- `SIG_SPAT { int Intersection_id; unsigned char movementName[5]; int signalGroup; int eventState; unsigned int minEndTime; }` — x86-64 기본 정렬 24바이트, `sig_SPaT[7]`=168바이트(LE, host order, htonl 없음).
- 송신은 1024바이트 datagram(앞 168B=데이터, 뒤 0 패딩). 수신측은 앞 168B만 사용.
- 하드코딩 라우팅 7개: sig_SPaT[0]=교차로1300/sg3, [1]=300/9, [2]=400/2, [3]=610/2, [4]=700/4, [5]=100/sg16, [6]=1500/sg1. 그 외 교차로/signalGroup 은 버려짐.
...
## 변경 이력
- **2026-06-17 모니터링 교차로 2개 추가** (`make` 빌드 확인): `sig_SPaT[5]`→`[7]` 확장, parseSpat 에 id 100(sg16)→[5], id 1500(sg1)→[6] 분기 + signalGroup 체크 추가, 종료 클리어 루프 `i<5`→`i<7`. wire 포맷 168B 로 증가 → 하류 `katri_obu_interface` 도 7개로 동기 필요.
