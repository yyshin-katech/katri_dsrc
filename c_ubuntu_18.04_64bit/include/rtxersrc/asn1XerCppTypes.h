...
class EXTERNXER ASN1XERDecodeBuffer : public ASN1XERMessageBuffer {
 protected:
   ...
   /**
    * The mInput union contains the three possible types of data that our
    * decode buffer can be constructed with: an input filename, an input
    * stream, or a memory buffer.
    */
   union {
      /**
       * The input file name that contains the XML data.
       */
      const char* fileName;
      /**
       * The input stream from which the decode buffer should receive
       * data.
       */
      OSRTInputStreamIF* pInputStream;

      /**
       * A structure describing the memory buffer.
       */
      struct {
         /**
          * A buffer to contain the input XML data.
          */
